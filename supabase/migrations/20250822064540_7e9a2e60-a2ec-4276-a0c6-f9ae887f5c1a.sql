-- Fix 1: Consolidate role resolution and remove privilege escalation
-- Drop the conflicting function that uses profiles table
DROP FUNCTION IF EXISTS public.get_current_user_role(uuid);

-- Update the main role function to be the single source of truth
...
-- Fix 2: Protect profiles.role from unauthorized changes
...
-- Fix 3: Enable RLS on leads_per_agent_per_month
...
-- Fix 4: Add missing validation triggers for leads
...
-- Fix 5: Add missing validation triggers for other tables
...
-- Fix 6: Add file validation triggers
