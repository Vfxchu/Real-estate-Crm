...
-- 1. Clean up duplicate get_current_user_role functions and fix role system
-- Drop the old function that might be causing conflicts
DROP FUNCTION IF EXISTS public.get_current_user_role(uuid);

-- Ensure we have the correct secure role function
...
-- Create the wrapper function that policies expect
...
-- 2. Fix properties RLS - remove the overly permissive policy
DROP POLICY IF EXISTS "properties_select_policy" ON public.properties;

-- Create a secure properties select policy
...
-- 3. Enable RLS on leads_per_agent_per_month and add policy
...
-- 4. Attach missing security triggers
-- Attach the profile role protection trigger
...
-- Attach the sanitize lead input trigger
...
-- Attach the file validation trigger
...
-- Also attach to contact_files for consistency
...
-- 5. Fix the role assignment validation trigger
...
-- 6. Clean up duplicate functions - remove the problematic one
DROP FUNCTION IF EXISTS public.create_property_with_files(text, text, text[], uuid);

-- 7. Ensure profile audit trigger is attached
