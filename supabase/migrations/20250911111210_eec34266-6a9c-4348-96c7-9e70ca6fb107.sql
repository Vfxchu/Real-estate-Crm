-- Fix security issues identified in the review (corrected version)
...
-- Users can update their own profile data (excluding role field)
...
-- 5. Add validation trigger to prevent role changes by non-admins
...
-- Create trigger to enforce role update validation
DROP TRIGGER IF EXISTS validate_profile_role_changes ON profiles;
